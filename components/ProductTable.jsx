...
const ProductTable = ({ products }) => {
    ...
    return (
        <div className="overflow-x-auto rounded-xl shadow-xl border border-gray-700 bg-gray-900 p-4">
                    ...
                    {products.map((product) => (
                        ...
                        >
                            ...
                            <td className="px-6 py-4 space-x-2">
                                <button onClick={()=> editProduct(product)} className="px-3 py-1 bg-purple-600 hover:bg-purple-700 text-white rounded-md text-sm transition">
                                    Edit
                                </button>
                                <button
            ...
            {editPopUp && (
                <ProductEdit product={product} onClose={()=> setEditPopUp(false)} id={product.id} />
            )}
        </div>
    );
};
